package grpcanalyticsserver.config;

import grpcanalyticsserver.service.UserAnalyticsServiceImpl;
    ...
    public void start() {
        try {
            server = ServerBuilder.forPort(grpcPort)
                    .addService((BindableService) new UserAnalyticsServiceImpl())
                    .build()
                    .start();

            running = true;
            log.info("gRPC-сервер запущен на порту {}", grpcPort);
            log.info("Сервис: UserAnalytics.AnalyzeUser()");
