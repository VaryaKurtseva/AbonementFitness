...
import org.springframework.amqp.support.converter.Jackson2JsonMessageConverter;  // ← обрати внимание: Jackson2JsonMessageConverter
 ...
 *
 * Этот сервис одновременно и consumer (слушает user.created),
 * и publisher (публикует user.enriched). Поэтому конфигурация включает:
 * - Exchange (общий для всех сервисов)
 * - Очередь для приёма user.created
...
public class RabbitMQConfig {
    public static final String ENRICHMENT_QUEUE = "q.enrichment.user-created";
    public static final String ENRICHMENT_DLQ = "q.enrichment.user-created.dlq";
    ...
    public Binding enrichmentBinding(Queue enrichmentQueue, TopicExchange eventsExchange) {
        return BindingBuilder
                ...
                .to(eventsExchange)
                .with(RoutingKeys.USER_CREATED); // только user.created
    }
